...
restService.post('/slack-test', function (req, res) {
    ...
    //var param1 = zpad(req.body.result.parameters.floor, 2);
    ...
    sql.connect(config, function (err) {
        ...
        request.input('floorID', sql.VarChar(2), param1);
        request.query('select top 5 ch_unit_id as [key],\'[]\' as [synonyms], rtrim(ch_unit_desc) as ch_unit_desc,mo_lp_with_cwtdisc1=CONVERT(varchar, CAST(mo_lp_with_cwtdisc AS money), 1) From mf_unit_status_details where ch_floor_id=@floorID', function (err, recordset) {
            if (err) console.log(err)
            ...
            for (var i = 0; i < len; i++) {
                itemObject = {
                    ...
                    "title": recordset.recordset[i].ch_unit_desc,
                    "image": {
